...
class Studentfee extends Admin_Controller
{
     ...
     public function collection_report()
    {
     ...
     if (!$this->rbac->hasPrivilege('collect_fees', 'can_view')) {
        ...
        $end_date=date('Y-m-d',strtotime($dates['to_date']));
         $data['collectlist']=$this->studentfeemaster_model->getFeeCollectionReport($start_date,$end_date,'','',$bank);        
        ...
        $data['results']=array();
            ...
            $bank = $_POST['bank']??'-';
       $data['results']=$this->studentfeemaster_model->getFeeCollectionReport($start_date,$end_date,$received_by,$group,$bank);  
    ...
    public function addstudentfee()
    {
        ...
        if($this->input->post('payment_mode') == 2){
            ...
            $this->form_validation->set_rules('check_number', $this->lang->line('check_number'), 'trim|required|xss_clean');
        }else if($this->input->post('payment_mode') == 3){
            $this->form_validation->set_rules('app_name', $this->lang->line('app_name'), 'trim|required|xss_clean');
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            $json_array               = array(
                ...
                'payment_mode' => $this->input->post('payment_mode'),
                'bank_name' => ($this->input->post('payment_mode') == "1" || $this->input->post('payment_mode') == "3" )?"":$this->input->post('bank_name'),
                'check_number' => $this->input->post('check_number')??NULL,
                'payment_app_name' => $this->input->post('app_name')??NULL,
            );
            $data = array(
