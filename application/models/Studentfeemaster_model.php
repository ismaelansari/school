...
class Studentfeemaster_model extends MY_Model
{
    ...
    {
       $sql= "SELECT `student_fees_master`.*,fee_groups.name,`student_fees_deposite`.payment_mode,`student_fees_deposite`.`check_number`,`student_fees_deposite`.`payment_app_name`,`student_fees_deposite`.`bank_name` FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id LEFT JOIN student_fees_deposite  on student_fees_master.id = student_fees_deposite.student_fees_master_id WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
        $query  = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
 ...
 public function getFeeCollectionReport($start_date, $end_date,$received_by=null,$group=null,$bank='-')
    {
        ...
        $this->db->join('bank', 'student_fees_deposite.bank_name=bank.id','left');
        if($bank != '-'){
            $this->db->where('student_fees_deposite.bank_name',$bank);
        }
               ...
               if($received_by!=null){
                ...
                if (!empty($return)) {
                    ...
                    foreach ($return as $r_key => $r_value) {
                        ...
                        $a['payment_mode']           = $value->payment_mode;
                        $a['bank_name_str']              = $value->bank_name_str;
                        $a['check_number']           = $value->check_number;
                        $a['payment_app_name']       = $value->payment_app_name;
