...
class Income_model extends My_Model {
    ...
    public function search($text = null, $start_date = null, $end_date = null,$bank = '-') {
        if (!empty($text)) {
            $this->db->select('income.id,income.date,income.name,income.invoice_no,income.amount,income.documents,income.note,income_head.income_category,income.inc_head_id,income.payment_mode,income.bank_name,income.check_number,income.payment_app_name,bank.name as bank_name_str')->from('income');
            $this->db->join('income_head', 'income.inc_head_id = income_head.id');
            $this->db->join('bank', 'income.bank_name = bank.id','left');
            $this->db->like('income.name', $text);
            if($bank != '-'){
                $this->db->where('income.bank_name',$bank);
            }
            $query = $this->db->get();
