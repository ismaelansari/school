...
class Expense_model extends My_Model {
     ...
     * @return mixed
     */
    public function search($text = null, $start_date = null, $end_date = null,$bank = '-') {
        if (!empty($text)) {
            $this->db->select('expenses.id,expenses.date,expenses.invoice_no,expenses.name,expenses.amount,expenses.documents,expenses.note,expense_head.exp_category,expenses.exp_head_id,expenses.payment_mode,expenses.bank_name,expenses.check_number,expenses.payment_app_name,bank.name as bank_name_str')->from('expenses');
            $this->db->join('expense_head', 'expenses.exp_head_id = expense_head.id');
            $this->db->join('bank', 'expenses.bank_name = bank.id','left');
            $this->db->like('expenses.name', $text);
            if($bank != '-'){
                $this->db->where('expenses.bank_name',$bank);
            }
            $query = $this->db->get();
            return $query->result_array();
        } else {
            $this->db->select('expenses.id,expenses.date,expenses.name,expenses.invoice_no,expenses.amount,expenses.documents,expenses.note,expense_head.exp_category,expenses.exp_head_id,expenses.payment_mode,expenses.bank_name,expenses.check_number,expenses.payment_app_name,bank.name as bank_name_str')->from('expenses');
            $this->db->join('expense_head', 'expenses.exp_head_id = expense_head.id');
            $this->db->join('bank', 'expenses.bank_name = bank.id','left');
    ...
    public function get($id = null) {
        $this->db->select('expenses.id,expenses.date,expenses.name,expenses.invoice_no,expenses.amount,expenses.documents,expenses.note,expense_head.exp_category,expenses.exp_head_id,expenses.id,expenses.payment_mode,expenses.bank_name,expenses.check_number,expenses.payment_app_name,bank.name as bank_name_str')->from('expenses');
        $this->db->join('expense_head', 'expenses.exp_head_id = expense_head.id');
        $this->db->join('bank', 'expenses.bank_name = bank.id','left');
        if ($id != null) {
